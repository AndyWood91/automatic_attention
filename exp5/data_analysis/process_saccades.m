function process_saccades(subjects, exclusions, ROIs, varargin)
% ProcessSaccades: I-VT Algorithm for determining saccade direction and
% latency from Tobii eyetracker data
%
% ProcessSaccades(subjects, exclusions ROIs, [fixationCoords], [discardAnticipatorySaccades], [discardOutsideFixationSaccades])
% subjectlist is a vector of participant numbers, e.g., [1:5 8 9:15].
%
% ANDY: added subject exclusions - set as [] for none, can't work out a better way
% to do this just yet
%
...
% gonna break without doing exclusions properly
for sub = 1:numel(subjects)
    
    summarySaccadeData = zeros(maxTrials,size(stimVector,1)+5+2); %initiate summary data array
    
    clc; disp(['Processing Subject ',num2str(subjects(sub))])
    
    subStep = subStep+1;
    
    GazeFileName = ['raw_data\Sub',num2str(subjects(sub))]; %CHANGE THIS FOR EACH ANALYSIS
    load(GazeFileName, 'DATA');
        
    for t = 1:480
        
        clc; disp(['Sub ', num2str(subjects(sub)), ' Trial ', num2str(t)]);
        
        %load data file for current trial
        GazeData = [double(cell2mat(DATA.stimEG(t,2))) double(cell2mat(DATA.stimEG(t,1)))];
        
        GazeData = double(GazeData);
        
        sampleInterval = ((GazeData(end,1)-GazeData(1,1))/1000)/size(GazeData,1); % determine the average interval between each successive eyetracker sample
        ...
        if discardOutsideFixation == 1
            ...
            if size(GazeData,1) > 80/sampleInterval
                for ss = 1 : round(80/sampleInterval)
                    % this part has to be setting the size of the fixation
        ...
        for s = sampleWindowLength+1:size(GazeData,1)-sampleWindowLength % step through trial data with a moving window of ~20ms (8 samples) 
            ...
            if sum(GazeData(s-sampleWindowLength:s+sampleWindowLength,8)==4)>0
                ...
                time = (GazeData(s+sampleWindowLength,1)-GazeData(s-sampleWindowLength,1))/1000000; %find time taken from beginning to end of time window in seconds
...
%               plot(time, GazeData(:,9)', time, GazeData(:,10)', time, [velocity 0 0 0], time, repmat(30,1,length(time)))
        ...
        if isempty(movList) == 0
            ...
            if isempty(idx) == 0
                ...
                % Andy: want to pull different reasons for discard
                discard_anticipatory = 0;
                discard_unfixated = 0;
                %if anticipatory saccade, or no samples near fixation within first 80ms, mark to be discarded
                
                % ANDY: this is the part I want to tweak
                % storing anticipatory discard separately
                if saccadeLatency < anticipationThreshold
                    discard_anticipatory = 1;
                else
                end
                
                % storing unfixed discard separately
                if withinCentre == 0
                    discard_unfixated = 1;
                else
                end
                
                if saccadeLatency < anticipationThreshold || withinCentre == 0 
                    discardTrial = 1;
                end
        ...
        summarySaccadeData(t,:) = [sub, t, saccadeLatency, direction, discardTrial ...
            discard_anticipatory, discard_unfixated];
        
        movList = {};
    end
    
    % TODO: build folder check    
    save(['processed_saccades\SummarySaccadeDataP',num2str(subjects(sub)),'.mat'],'summarySaccadeData');
